 ...
 */
	...
	$scope.Init=function() {
         ...
         $scope.checkSubItem=function(i) {
             if (i === '') return 0;
             if ($scope.sideMenu()[i].index === $scope.curIndex) {return 1;
             } else {
                 return 0;
             };
         };
        ...
        $scope.setLangFile=function() {
          ...
          if ($location.url() === '/') {url='data/lang_main.json';};//Main page has different URL syntax than other
  ...
  //  console.log('fruitsController async returned value');
   ...
   // nService.setImageObject(results.pam);
        ...
        $scope.fillIMG=function(num) {
         ...
         $scope.getModalID=function() {
           ...
           if (url === '/gconcrete') { return '#gconcrete' };
           if (url === '/gphoto') { return '#gphoto' };
         };
        ...
        $scope.setPicture=function(item) {
            ...
            if ($scope.imgWidth < 350 ) {$scope.imgWidth=350;};
        ...
        if ( index === -1 ) { index=0;}; // if index=0 first link in side menu is highlihgted. 
   ...
   //     alert("Total objects="+$scope.getKeysCount($scope.allimages));
        ...
        $scope.setPlinks=function() {
            ...
            if ($scope.allimages.pam.length%pageItems > 0) {immP=1;}; 
            
            if (url === '/gconcrete') {
                len=$scope.allimages.con.length;
                if ($scope.allimages.con.length%pageItems > 0) {immP=1;}; 
            };
            
            if (url === '/gphoto') {
                len=$scope.allimages.fok.length;
                if ($scope.allimages.fok.length%pageItems > 0) {immP=1;}; 
            };
